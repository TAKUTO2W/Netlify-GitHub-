...
function Get-EventDetailFields($html) {
    ...
    if ($f.venue) {
        ...
        # 収集元が付けた「[ 長野県 ] 蓼科園地」のような接頭辞を落とす。
        # これを残すとサイト上で「長野県 / [ 長野県 ] 蓼科園地」と二重表示になる。
        # ※「愛媛県紙産業技術センター」のように県名が正式名称の一部のものは
        #   角括弧が無いので、この処理では触らない（削ると別の名前になってしまう）。
        $f.venue = ($f.venue -replace '^\s*[\[［]\s*[^\]］]{2,6}[\]］]\s*', '').Trim()
