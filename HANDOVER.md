...
**A. note用アイキャッチ画像の準備（ユーザー指示済み・未着手）**
- ユーザーの要望: 「アイキャッチは基本的に `images/hero-car.jpg`（トップのヒーロー画像）を使って」
- やること: hero-car.jpg（1536×1024・2.7MB）を **note推奨の1280×670に切り出して軽量化**し
  `images/note-eyecatch.jpg` として保存 → 金曜の下書きファイル冒頭にそのパスを明記
- ⚠️ **noteへの画像アップロードは自動化不可**（OSのファイル選択ダイアログが開くため）。
  設定はユーザーの手作業になる。その前提で「選ぶだけ」の状態にしておくのがゴール

**B. はてなブログの「placeholder」記事削除（未処理）**
